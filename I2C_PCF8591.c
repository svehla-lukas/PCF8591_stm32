...
// Conversion macros
#define ADC_CONVERT(raw_value, ref_voltage) ((raw_value) * (ref_voltage) / ADC_PCF_RESOLUTION)
...
HAL_StatusTypeDef I2C_PCF8591_init(uint8_t enableAnalogOutput, uint8_t inputMode, float reference_voltage) {
    result = HAL_ERROR;
    ref_voltage = reference_voltage;

    // Check if the device is ready
    result = HAL_I2C_IsDeviceReady(&hi2c1, PCF8591_address << 1, 1, HAL_MAX_DELAY);
    if (result != HAL_OK) {
        return result;
    }
    ...
    tx_data[0] = (tx_data[0] & ~TX_INPUT_MODE_MASK) | inputMode | enableAnalogOutput;

    // Transmit configuration
    result = HAL_I2C_Master_Transmit(&hi2c1, PCF8591_address << 1, tx_data, size_1, HAL_MAX_DELAY);
    return result;
}
...
HAL_StatusTypeDef I2C_PCF8591_write_ain(uint8_t setValue) {
    ...
    result = HAL_I2C_Master_Transmit(&hi2c1, PCF8591_address << 1, tx_data, sizeof(tx_data), HAL_MAX_DELAY);
    return result;
}
...
uint8_t* I2C_PCF8591_read_raw_analog_ch(uint8_t channel) {
    ...
    result = HAL_I2C_Master_Receive(&hi2c1, PCF8591_address << 1, rx_data, sizeof(rx_data), HAL_MAX_DELAY);
    if (result != HAL_OK) {
        return NULL;
    }
